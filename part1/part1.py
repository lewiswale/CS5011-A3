...
def train_classifier(file_to_use):
    ...
    X = pd.read_csv(file_to_use, usecols=X_cols)        #Reading only question columns
    df_countries = pd.read_csv(file_to_use, usecols=[column_count-1])   #Reading only location column
